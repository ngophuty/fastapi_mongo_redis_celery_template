...
@router.post('/create_profile_user', dependencies=[Depends(validate_token)])
async def create_profile_user(request: RequestUserProfile):
    create = await create_profile(request)
    return create


@router.post('/get_profile_user', dependencies=[Depends(validate_token)])
async def get_profile_user():
    data = await get_profile()
    return data


@router.post('/update_profile_user', dependencies=[Depends(validate_token)])
async def update_profile_user(request: RequestUserProfile):
    update = await update_profile(request)
    return update


@router.post('/delete_profile_user', dependencies=[Depends(validate_token)])
